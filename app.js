...
function buildLayout(c){
  ...
  function renderTextList(options={}){
    ...
    const preservePagePosition=bringSelectedRegionIntoView||bringSelectedTextIntoView;
    const pageScrollX=preservePagePosition?window.scrollX:0;
    const pageScrollY=preservePagePosition?window.scrollY:0;
    ensureStateCompatibility();
    ...
    displayTexts.forEach((text,index)=>{
      ...
      const selectCard=()=>{
        ...
        state.selectedTextId=text.id;
        activeTextarea=textarea;
        const cards=Array.from(list.querySelectorAll(".text-card"));
        cards.forEach((node)=>{
          const isSelected=node.dataset.textId===text.id;
          const isSelectedRegion=Boolean(state.selectedRegionId&&node.dataset.regionId===state.selectedRegionId);
          node.classList.toggle("active",isSelected);
          node.classList.toggle("selected-text-priority",isSelected);
          node.style.order=isSelected?"-2":isSelectedRegion?"-1":"0";
        });
        if(changed){
          const visualOrder=[
            ...cards.filter((node)=>node.dataset.textId===text.id),
            ...cards.filter((node)=>node.dataset.textId!==text.id&&state.selectedRegionId&&node.dataset.regionId===state.selectedRegionId),
            ...cards.filter((node)=>node.dataset.textId!==text.id&&(!state.selectedRegionId||node.dataset.regionId!==state.selectedRegionId))
          ];
          visualOrder.forEach((node,position)=>{
